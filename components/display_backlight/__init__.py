...
@automation.register_action(
    "display_backlight.turn_on", TurnOnAction, TOGGLE_ACTION_SCHEMA
)
async def display_backlight_turn_on_to_code(config, action_id, template_arg, args):
    paren = await cg.get_variable(config[CONF_ID])
    return cg.new_Pvariable(action_id, template_arg, paren)
@automation.register_action(
...
def to_code(config):
    cg.add_global(display_backlight_ns.using)
